(function(){
  ...
  window.SheetStorage={
    blankSheet,
    load(){try{const value=JSON.parse(localStorage.getItem(KEY));if(value&&value.version===1)return {version:1,template:['blank','nagare','tenkey'].includes(value.template)?value.template:'blank',current:{...blankSheet(),...(value.current||{})},sheets:Array.isArray(value.sheets)?value.sheets:[]};}catch(e){}return {version:1,template:'blank',current:blankSheet(),sheets:[]};},
